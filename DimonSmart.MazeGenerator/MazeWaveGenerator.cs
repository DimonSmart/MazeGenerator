...
public class MazeWaveGenerator(IMaze maze, IWavePlotter? wavePlotter = null) : IMazeWaveGenerator
{
    ...
    public MazeWave GenerateWave(int startX, int startY, Func<int, int, bool> criteria,
    ...
    public Task<MazeWave> GenerateWaveAsync(int startX, int startY, Func<int, int, bool> criteria,
        CancellationToken cancellationToken = default)
    {
        return GenerateWaveCoreAsync(startX, startY, criteria, SetWavePointAsync, cancellationToken);
    }

    public MazeWave GenerateWave(int startX, int startY, int endX, int endY,
        CancellationToken cancellationToken = default)
    {
        return GenerateWave(startX, startY, (x, y) => x == endX && y == endY, cancellationToken);
    }

    public Task<MazeWave> GenerateWaveAsync(int startX, int startY, int endX, int endY,
        CancellationToken cancellationToken = default)
    {
        return GenerateWaveAsync(startX, startY, (x, y) => x == endX && y == endY, cancellationToken);
    }

    private async Task<MazeWave> GenerateWaveCoreAsync(int startX, int startY, Func<int, int, bool> criteria,
        Func<int, int, int, CancellationToken, Task> setWavePoint, CancellationToken cancellationToken)
    {
        ...
        do
        {
            while (stack1.Any())
            {
                ...
                if (criteria(x, y))
                {
                    return new MazeWave(startPoint, new Point(x, y), _wave);
                }
