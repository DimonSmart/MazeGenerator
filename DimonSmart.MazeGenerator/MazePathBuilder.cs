namespace DimonSmart.MazeGenerator
{
    public class MazePathBuilder(MazeWave mazeWave, IPathPlotter? pathPlotter = null)
    {
        ...
        private static Point? GetPreviousPoint(Point point, int[,] wave)
        {
            ...
            if (x > 0 && wave[y, x - 1] == waveNumber - 1) return new Point(x - 1, y);
            if (x < wave.GetLength(1) - 1 && wave[y, x + 1] == waveNumber - 1) return new Point(x + 1, y);
            if (y > 0 && wave[y - 1, x] == waveNumber - 1) return new Point(x, y - 1);
            if (y < wave.GetLength(0) - 1 && wave[y + 1, x] == waveNumber - 1) return new Point(x, y + 1);
